...
import { createPublicSubmission, isSupabaseConfigured } from "../supabase";
...
export const ContributeView: React.FC<ContributeViewProps> = ({ onAddGap, onNavigate }) => {
  const hasRemoteSubmission = isSupabaseConfigured();
  ...
  const [successMode, setSuccessMode] = useState<"remote" | "local">("local");
  ...
  const buildSubmissionContent = (newGap: InformationGap) => {
    const content = [
      `分类：${newGap.categoryLabel}`,
      `地区：${newGap.region}`,
      `一句话说明：${newGap.tagline}`,
      `这是什么：${newGap.description}`,
      `为什么值得知道：${newGap.coreGap}`,
      `适合谁先了解：${newGap.appliedTo}`,
      `成本：${newGap.costEstimation}`,
      `时间窗口：${newGap.timeline}`,
      `使用条件/申请条件：${newGap.practicalSteps.join(" / ")}`,
      `风险与误区：${newGap.risksAndWarmings}`,
      `路径示例：${newGap.scenarios}`,
    ].join("\n");

    return content.slice(0, 2900);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (hasRemoteSubmission) {
        await createPublicSubmission({
          type: "submission",
          title: newGap.title.slice(0, 120),
          content: buildSubmissionContent(newGap),
          sourceUrl: linkUrl1,
        });
        setSuccessMode("remote");
      } else {
        onAddGap(newGap);
      ...
      onAddGap(newGap);
      setSuccessMode("local");
      setValidationError("公开提交暂时失败，已保存到本机。你可以稍后再提交一次。");
      setIsSuccess(true);
    } finally {
      ...
      {isSuccess ? (
        ...
        <div className="bg-white rounded-3xl p-10 border border-gray-100 shadow-xs text-center max-w-xl mx-auto space-y-5 animate-scale-up">
          ...
          <h2 className="text-2xl font-extrabold text-gray-950">
            {successMode === "remote" ? "已提交待核验" : "已保存到本机"}
          </h2>
          <p className="text-xs md:text-sm text-text-muted leading-relaxed">
            {successMode === "remote"
              ? "内容已保存到公开数据库的待核验列表，正式公开前需要人工核验，不会自动进入图谱。"
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-6 md:p-8 border border-gray-100 shadow-xs space-y-6">
          ...
          <div className="p-4 rounded-2xl bg-primary/4 border border-primary/10 text-xs text-primary font-semibold flex items-start gap-2">
            ...
            <span>
              {hasRemoteSubmission
                ? "提交后会进入待核验列表，正式公开前需要人工核验；请拒绝虚假、夸大或无凭据的投稿。"
                : "当前提交内容仅保存在本机，暂不会公开发布。正式公开前需要人工核验；请拒绝虚假、夸大或无凭据的投稿。"}
            </span>
          </div>
                ...
                {steps.map((step, idx) => (
                  <div key={idx} className="flex items-center gap-2 animate-scale-up">
                    ...
                    <button
                      ...
                      className="h-9 w-9 border border-red-100 bg-red-50 hover:bg-red-100 text-red-500 rounded-lg flex items-center justify-center shrink-0 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-red-50 select-none cursor-pointer"
                    >
          ...
          <div className="pt-4 border-t border-gray-100 text-right">
            <button
              ...
              className="px-8 py-4 rounded-xl bg-primary hover:bg-primary-container text-white font-extrabold text-sm shadow-md shadow-primary/10 transition-all cursor-pointer select-none disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isSubmitting ? "提交中..." : hasRemoteSubmission ? "提交待核验" : "保存到本机"}
            </button>
          </div>
        </form>
      )}
